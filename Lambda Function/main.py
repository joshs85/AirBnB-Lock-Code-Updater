    ...
    try:
        ...
        for event in c.events:
            if not lock_code_updated:
                ...
                print "Comparing Now:{} to EventBegin:{} with EventEnd:{}".format(now, begin, end)
                if now.date() == begin.date():
                    phone = get_phone(description)
                    lock_code = phone[-4:]
                    print "Setting lock code to {0}".format(lock_code)
